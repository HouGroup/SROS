...
class SRO:
    ...
    def exchange_OF(
        ...
        """
        Perform exchange of O and F once.
        """
        random.seed(random_seed)
        ...
        target = True
        while True:
            if self.cnn.get_cn_dict(self.structure, a_site).get('Li', 0) >= self.cnn.get_cn_dict(self.structure,
                                                                                                 b_site).get('Li', 0):
                break
                # 即如果F周围的Li多于O周围的Li，则保持target=True并进入下一步
            else:
                target = False
                # 即如果F周围的Li少于O周围的Li，则使target=False并进入下一步
                break

        if random.random() < prob:  # 结构F周围的Li比目标值少
            if not target:
                self.exchange_site(a_site, b_site)
    ...
    def run(self, max_steps: int,
        ...
        random.seed(random_seed)
        print("Innitial alpha_FLi:", self.a, "Innitial F-Li:", self.get_neighbor(self.anion_a, self.cation))
        ...
        for i in range(max_steps):
            ...
            if abs(self.alpha_LiLi() - target_alpha_LiLi) <= tol:
                print("Target alpha_LiLi reached", "Final Li-Li:", self.get_neighbor_LiLi())
                break

        for i in range(max_steps):
            self.exchange_OF(target_alpha, rate, random_seed=random.randrange(1000))
            print("New alpha_FLi:", self.alpha(), "F-Li:", self.get_neighbor(self.anion_a, self.cation))
            if abs(self.alpha() - target_alpha) <= tol:
                print("Target alpha_LiLi reached", "Final F-Li:", self.get_neighbor(self.anion_a, self.cation))
                return "Target alpha_LiLi and alpha reached"
