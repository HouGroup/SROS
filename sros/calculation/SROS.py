...
class SRO:
    ...
    @staticmethod
    def _is_reached(value: Union[int, float], target: Union[int, float], tolerance: Union[int, float]) -> bool:
    ...
    def run_lif(self, max_steps: int,
            ...
            # target_alpha_LiLi: Union[int, float] = 0,
            rate: Union[int, float] = 1,
            tol: Optional[Union[int, float]] = None,
            random_seed: Union[None, int] = None,
            tolerance: Union[int, float] = 0.05,
            ):
        ...
        self.a = self.alpha_fix()
        reached = self._is_reached(self.a, target_alpha, tolerance)
        if reached:
            print("target alpha_LiF reached.")
        else:
            print(
                "Target alpha_LiF not reached. "
                f"Current alpha: {self.a}; target: {target_alpha}; tolerance: {tolerance}"
            )
        return {
            "alpha": float(self.a),
    ...
    def run(self, max_steps: int,
        ...
        self.a = self.alpha_fix()
        self.a_LiLi, self.a_LiLi_dict = self.alpha_LiLi()
        alpha_reached = self._is_reached(self.a, target_alpha, tolerance)

        lili_steps = 0
        if alpha_reached:
            for i in range(max_steps):
                if self._is_reached(self.a_LiLi, target_alpha_LiLi, tolerance):
            ...
            print("Skipping alpha_LiLi tuning because alpha_LiF target was not reached.")

        self.a_LiLi, self.a_LiLi_dict = self.alpha_LiLi()
        alpha_LiLi_reached = self._is_reached(self.a_LiLi, target_alpha_LiLi, tolerance)
        all_reached = alpha_reached and alpha_LiLi_reached

        if all_reached:
            print("Both target alpha and alpha_LiLi reached.")
        else:
            if alpha_reached and not alpha_LiLi_reached:
                print(
                    "Target alpha_LiLi not reached. "
                    f"Current alpha_LiLi: {self.a_LiLi}; "
                    f"target: {target_alpha_LiLi}; tolerance: {tolerance}"
                )
        return {
            "alpha": float(self.a),
            "target_alpha": target_alpha,
            "alpha_reached": alpha_reached,
            "alpha_LiLi": float(self.a_LiLi),
            "target_alpha_LiLi": target_alpha_LiLi,
            "alpha_LiLi_reached": alpha_LiLi_reached,
            "all_reached": all_reached,
            "tolerance": tolerance,
            "alpha_steps": alpha_steps,
            "alpha_LiLi_steps": lili_steps,
        }
